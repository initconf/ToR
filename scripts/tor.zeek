...
event line(description: Input::TableDescription, tpe: Input::Event, left: tor_Idx, right: tor_Val)
{
...
{
        Input::add_table([$source=tor_feed, $name="tor_table", $idx=tor_Idx, $val=tor_Val,  $destination=tor_table, $mode=Input::REREAD,$ev=line ]);
}
